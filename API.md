...
- **参数:**
    ...
    - `template` (numpy.ndarray, optional): 自定义模板图像。传入后**不使用** COCO 特征模板，且**不做自动缩放**
      （COCO 特征模板在加载时会按当前屏幕分辨率自动缩放，而 `template` 原样使用），调用方需自行保证模板尺寸
      匹配当前分辨率。常规做法：`get_feature_by_name(name).mat` 取已缩放的模板，处理后传入。
      注意：传入 `template` 后框架不再校验特征是否存在，特征缺失时需自行守卫。
    - `frame_processor` (callable, optional): 匹配前对 **`box` 裁剪出的搜索区域**做自定义处理（如二值化）。
      与 `template` 配合、对帧区域和模板做同一种变换，可实现变换域匹配
      （如二值化识图：`template=binarize_image(feature.mat, t)` +
      `frame_processor=lambda img: binarize_image(img, t)`，只比亮部形状，对颜色偏移鲁棒）。
- **返回:**
