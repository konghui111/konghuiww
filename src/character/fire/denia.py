...
CHARACTER_NAME = "denia"  # 角色名, 对应 COCO 标注中的 category 后缀
...
def _action_r1(task):
    while task.enabled and task._combat_active:
        if check_skill_available(task, "r",skill_image="denia_r1"):
            break
        time.sleep(0.05)
    while task.enabled and task._combat_active:  # 持续按 r 直到 r1 消失
        task.send_key("r")
        time.sleep(0.05)
...
def _action_ee(task):
    while task.enabled and task._combat_active:  
        ...
        time.sleep(0.05)
    while task.enabled and task._combat_active:  
        if check_skill_available(task, "e",skill_image="denia_super_e2"):
            break
        task.send_key("e")
        time.sleep(0.05)    
    while task.enabled and task._combat_active:  # 持续按 e 直到 super_e2 消失
        task.send_key("e")
        time.sleep(0.05)
        if not check_skill_available(task, "e",skill_image="denia_super_e2"):
            break
    return True
register_action(CHARACTER_NAME, "ee")  # 注册动作 eer2

def _action_r2(task):
    while task.enabled and task._combat_active:  
        if check_skill_available(task, "r",skill_image="denia_r2"):
            break
        task.click()
        time.sleep(0.05)    
    while task.enabled and task._combat_active:  # 持续按 r 直到 r2 消失
        task.send_key("r")
        time.sleep(0.05)
