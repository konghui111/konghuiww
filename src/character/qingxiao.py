...
CHARACTER_NAME = "qingxiao"  # 角色名, 对应 COCO 标注中的 category 后缀
...
def _action_main(task):
    ...
    while task.enabled and task._combat_active:
        task.send_key("e")
        time.sleep(0.07)  # 轮询间隔
        if not check_skill_available(task, "e", skill_image="qingxiao_e"):  
            break
    #按住左键
    task.mouse_down()
    while task.enabled and task._combat_active:
    ...
    while task.enabled and task._combat_active:
        task.send_key("r")
        time.sleep(0.07)  # 轮询间隔
        if not check_skill_available(task, "r", skill_image="qingxiao_r"):  
            break
    task.mouse_up()
    return True
