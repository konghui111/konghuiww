...
CHARACTER_NAME = "xigelika"  # 角色名, 对应 COCO 标注中的 category 后缀
...
def _action_main(task):
    ...
    while task.enabled and task._combat_active:  
        if check_skill_available(task, "a",skill_image="xigelika_z1"):
            break
        task.click()
        time.sleep(0.07)  
    task.mouse_down()
    ...
    time.sleep(0.4)     
    while task.enabled and task._combat_active:  
        if not check_skill_available(task, "a",skill_image="xigelika_z1"):
            break
        time.sleep(0.05) 
    task.mouse_up()
    #qr     
    while task.enabled and task._combat_active:  
        if check_skill_available(task, "r",skill_image="xigelika_r"):
            break
        time.sleep(0.05)  
    while task.enabled and task._combat_active:  
        task.send_key("q")
        task.send_key("r")
        if not check_skill_available(task, "r",skill_image="xigelika_r"):
            break
        time.sleep(0.05)          
    ...
    #a到重击出现
    while task.enabled and task._combat_active:  
        if check_skill_available(task, "a",skill_image="xigelika_z2"):
            break
        task.click()
        time.sleep(0.07)
    #长按攻击+e    
